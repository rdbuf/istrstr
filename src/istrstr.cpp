...
__m256i toupper(__m256i str) {
    static const __m256i DF = _mm256_set1_epi8(0xDF);
    return _mm256_and_si256(str, DF);
}
...
std::vector<uint64_t> find_all(const char *text, uint64_t text_n, const char target[4]) {
    ...
    const __m256i tgt = toupper(_mm256_loadu_si256(reinterpret_cast<const __m256i_u *>(&tgt_struct)));

    // after each `i` we must have at least 35 bytes available
    const uint64_t loop_n = (text_n >= 35) * (((text_n - 35) / 29) * 29);
    const uint64_t remainder_n = text_n - loop_n;

    for (int i = 0; i < loop_n; i += 29) {
        // Prepare 4 source match vectors w/ different offsets
        const __m256i src0 = _mm256_loadu_si256(reinterpret_cast<const __m256i_u *>(&text[i + 0]));
        const __m256i res0 = _mm256_cmpeq_epi32(toupper(src0), tgt);

        const __m256i src1 = _mm256_loadu_si256(reinterpret_cast<const __m256i_u *>(&text[i + 1]));
        const __m256i res1 = _mm256_cmpeq_epi32(toupper(src1), tgt);

        const __m256i src2 = _mm256_loadu_si256(reinterpret_cast<const __m256i_u *>(&text[i + 2]));
        const __m256i res2 = _mm256_cmpeq_epi32(toupper(src2), tgt);

        const __m256i src3 = _mm256_loadu_si256(reinterpret_cast<const __m256i_u *>(&text[i + 3]));
        const __m256i res3 = _mm256_cmpeq_epi32(toupper(src3), tgt);
    ...
    }
    {
        ...
        __m256i res0 = _mm256_cmpeq_epi32(toupper(src0), tgt);
        ...
        __m256i res1 = _mm256_cmpeq_epi32(toupper(src1), tgt);
        ...
        __m256i res2 = _mm256_cmpeq_epi32(toupper(src2), tgt);
        ...
        __m256i res3 = _mm256_cmpeq_epi32(toupper(src3), tgt);
